...
import SubscribersActionsBar from '@/components/subscribers/bar/SubscribersActionsBar.vue';
...
const searchDescription = "Поиск по почте..."
...
async function fetchFriends() {
    ...
    try {
        const response = await axios.post(`${API_CONFIG.BASE_URL}/api/v1/subscriber/all`, {
            search: state.search,
            sortBy: state.sortBy,
...
<template> 
  ...
  <SubscribersActionsBar @success="onRefresh" />

    <SubscribersList v-if="!isLoading && !error" 
        :items="state.items" 
